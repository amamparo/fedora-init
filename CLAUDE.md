...
- `files/` — assets the modules install verbatim (TLP drop-in config, zshrc, the GNOME extensions, the GDM dconf drop-in).
...
- 80-login-keyring: the login keyring's key is derived from the login password, so fingerprint and auto-login can never unlock it (`/etc/pam.d/gdm-fingerprint` has no `pam_gnome_keyring` line; the exact prompt wording is diagnostic — "did not get unlocked" means PAM never tried, "no longer matches" means it tried and the keyring password is stale). The fix writes GDM's *system* dconf db, the one config only the greeter reads (`DCONF_PROFILE=gdm`) — the in-session lock screen resolves the same key against *user* dconf and sudo/polkit use authselect's `system-auth`, so their fingerprint survives. Unlike the rest of the repo's dconf writes, this one MUST run under sudo. Never create `/etc/dconf/profile/gdm`: Fedora's gdm rpm ships `/usr/share/dconf/profile/gdm` (with `system-db:gdm`), and a file in /etc would shadow it. The lock file is load-bearing — `user-db:user` comes *first* in that profile, so an unlocked key could be overridden from `/var/lib/gdm/.config/dconf/user`. `dconf update` recompiles every system db unconditionally (the mtime gate died in dconf's 2019 C rewrite — don't add a `touch`), which is why the module only runs it when a `cmp -s` guard saw a drop-in change. `authselect disable-feature with-fingerprint` is the wrong tool here: it gates only sudo/polkit fingerprint, not the greeter's unconditional gdm-fingerprint stack.
